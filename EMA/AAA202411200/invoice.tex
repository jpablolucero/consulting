...
\invoiceref{AAA202411200} % Invoice number
...
\makecvfooter
  ...
  \begin{invoicetable}
    ...
    \invoiceitem{
      ...
      Study of the interactions in reflection, sheath and
      repartitions. The complexity of the decision tree is
      substantial. It will need to be addressed carefully before
      getting any gains.
    }{180}{5}
    \invoiceitem{
      \texttt{\bf EMA3D-1418} \vspace{0.1cm}\\

      Got a better understanding of the decision tree. There are
      definitely simplifications to be done but still need a deeper
      study. Shifting to ScatterCharge for now.
    }{180}{5}
    \bf{November} & & & \\\cline{1-1}
    \invoiceitem{
      \texttt{\bf EMA3D-1418} \vspace{0.1cm}\\

      Had issues with SonicWall VPN, called Synagex and they couldn’t
      find a solution unfortunately. Problem continued during the
      weekend and solved itself somehow on Monday Nov 4.      
    }{60}{5}
    \invoiceitem{
      \texttt{\bf EMA3D-1418} \vspace{0.1cm}\\

      {\bf 20\%} speedup achieved for \texttt{PIC::UpdateNew(int)} by
      optimizing \texttt{PIC::ScatterCharge(Uint)} and
      \texttt{TE4SubStruct::Values(vector <double>)}.
      Reported to Bryon in JIRA comment.
    }{240}{5}
    \invoiceitem{
      \texttt{\bf EMA3D-1418} \vspace{0.1cm}\\

      New timings and reporting back to Bryon on a JIRA comment.
    }{60}{5}
    \invoiceitem{
      \texttt{\bf EMA3D-1418} \vspace{0.1cm}\\

      Sub-timing of reflections required dedicated timing routine not
      to affect the overall time too much.  Results show in Reflections,
      {\bf 43\%} is spent in
      \texttt{PIC::CurrentElement(vector,vector,Uint)} and {\bf 27\%} in
      \texttt{PIC::Reflections(Uint,double)}
    }{60}{5}
    % \invoiceitem{
    %   \texttt{\bf EMA3D-1418} \vspace{0.1cm}\\
    % }{}{5}
