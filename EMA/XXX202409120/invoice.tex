...
\hypersetup{%
  pdftitle={J. P. Lucero Lorca - XXXXXXX},
...
\invoiceref{XXX202409120} % Invoice number
\invoiceissued{\today} % Date issued
\invoicedue{NOT A BILL} % Due date, a number of days into the future, use 0 for due on receipt
...
\makecvfooter
  ...
  \begin{tabular}{p{0.24\textwidth} p{0.22\textwidth} p{0.22\textwidth} p{0.22\textwidth}}
    ...
    \multirow{2}{*}{{\color{highlightcolour} \Huge XXXXXXXX}} & \textbf{Recipient} & \\
  ...
  \begin{invoicetable}
    \bf{August} & & & \\
    \invoiceitem{Backlog of configurations required to run charge on Linux. 
      ...
      General overlook of data structures in VectorSolve , identify coverage of the given test, time properly. }{180}{5.0}
    \bf{September} & & & \\
    \invoiceitem{Time measurements and determination of the time bottlenecks in FemQS::VectorSolve. Discovery of main bottleneck due to the usage of CSRFormat data structures that was underperformant in the past when working with FemMatrix::parvector to optimize FemMatrix::BiCGStabVectorSolve}{120}{5.0}
    \invoiceitem{New FemQS.cpp file created with loop simplification to improve vectorization. Measurements give 70\% speed-up on FemQS::UpdateGlob and 40\% speed-up on FemQS::VectorSolve.}{120}{5.0}
    \invoiceitem{Microsoft Teams chat, delivered new FemQS.cpp for testing, explained changes involved and impact of loop simplifications. Agreed on concentrating in BuildForce, BuildGrad and GradSolve!.}{65}{5.0}
    \invoiceitem{Study of FemQS::BuildForce. Identification of critical loop and assignments. Findings show a high amount of branching, recommendations issued.}{120}{5.0}
    \invoiceitem{Microsoft Teams conversation about the usage of MPI\_AllReduce in ReadNew::PIC::VectorAll. Recommeded usage of MPI\_Reduce instead to reduce operations by a factor of 2 since no redistribution of the final result is needed for writing results to disk.}{30}{5.0}
  \end{invoicetable}
